...
function f6() {
    let kluch = document.querySelector('.i-61').value;
...
function f18() {
    ...
    if (a18[val] !== undefined) {
        for (let i = 0; i < a18[val].length; i++) {
            out += a18[val][i] + ' ';
        }
    }
...
function f19() {
    let val = document.querySelector('.i-19').value.toLowerCase();
    let out = '';
    for (let key in a19) {
        for (let i = 0; i < a19[key].length; i++) {
            if (a19[key][i].toLowerCase() == val) {
                out += key;
            }
        }
    }
